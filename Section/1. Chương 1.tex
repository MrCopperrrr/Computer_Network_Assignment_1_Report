...
\begin{itemize}
    \item \textbf{HTTP Server và Proxy cơ bản:} Xây dựng thành công máy chủ HTTP từ thư viện \textit{socket} cấp thấp, có khả năng xử lý yêu cầu GET và POST. Hệ thống cũng bao gồm Proxy Server đơn giản để chuyển tiếp request HTTP, minh họa cho kiến trúc nhiều lớp trong ứng dụng web thực tế.
    ...
    \item \textbf{Giao thức Client-Server cho việc khám phá Peer:} Cho phép Peer Client gửi thông tin (địa chỉ IP, port) lên Tracker Server để đăng ký, đồng thời truy vấn danh sách các peer khác đang trực tuyến.
    ...
    \item \textbf{Tích hợp giao diện Web:} Xây dựng giao diện người dùng bằng HTML, CSS và JavaScript, mang lại trải nghiệm chat trực quan và thân thiện.
    ...
    \item \textbf{Duy trì trạng thái đăng nhập phía Client:} Lưu thông tin phiên làm việc bằng \textit{localStorage}, giúp người dùng tự động kết nối lại mà không cần thao tác thủ công, nâng cao trải nghiệm sử dụng.
\end{itemize}
